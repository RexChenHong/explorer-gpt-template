...
本儲存庫提供一套**方法論模板**，用於協調多個大型語言模型（LLMs）  
...
- Individual developers building **quantitative trading systems** or other  
  ...
  高資料量系統、希望**從大型開源專案中學習**  
  (like vn.py, Polars, etc.) **without copying code** and **without burning  
  （如 vn.py、Polars 等），但**不想直接複製程式碼**、也**不想大量燒 token**  
...
A real-world example of high-end model evaluation of extracted patterns.
一份高階模型對前述萃取結果進行評估的真實範例。
It turns them into general design rules and trade-offs
並將其轉化為通用的設計原則與取捨判斷，
...
when exploring large repositories.
用於探索大型開源專案。
...
Take the examples as reference outputs:
將 examples 視為「參考輸出結果」：

Use them to calibrate what your low-tier and high-tier prompts should produce.
用來校準低階與高階模型輸出應該達到的深度與範圍。

For your own target repository:
套用到你自己的目標專案時：

Start from the GitHub URL
從 GitHub 專案網址開始

Follow the Phase-based workflow in EXPLORER_GPT.md
依照 EXPLORER_GPT.md 中的 Phase 流程執行

Replace Polars with your target project
將 Polars 換成你的目標專案

Optionally adapt the final rules into your own agent skills
...
The examples illustrate the workflow itself,
範例僅用於示範探索流程本身，
not the latest internal implementations of those projects.
並非用來描述專案的最新內部實作。
